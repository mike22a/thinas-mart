...
function set_cookies($config) {
    ...
    return setcookie($config['sess_cookie_name'], $config['encryption_key'], time() + $config['sess_expiration'], '/');
}
...
function session_flash($name = "", $message = "", $class = "alert alert-success") {
    session_start();
    if (!empty($name)) {
        if (!empty($name) && !empty($message)) {
            ...
            echo '<div class="' . $_SESSION[$name . "_class"] . '"  role="alert">' . $_SESSION[$name] . '</div>';
            unset($_SESSION[$name]);
            unset($_SESSION[$name . "_class"]);
        }
    }
